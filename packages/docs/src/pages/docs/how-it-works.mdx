...
Surimi is for you if you want to write CSS (using typescript). It's even more for you, if you write super complex CSS and need better maintainability. It's not a tailwind alternative though. We're not replacing CSS.
...
For the nerds: Surimi is a query-builder API that assembles a CSS [AST](https://en.wikipedia.org/wiki/Abstract_syntax_tree) via a custom parser/builder,
...
5. The compiler loads the file (and it's imports) using [rolldown](https://rolldown.rs/).
   ...
   5.2. It resolves all imports (currently with limitations, see [github issues](https://github.com/surimidev/surimi/issues))
